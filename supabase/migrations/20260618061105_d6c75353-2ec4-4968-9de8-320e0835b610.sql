DROP FUNCTION IF EXISTS public.claim_package_daily_income(uuid);

CREATE OR REPLACE FUNCTION public.claim_package_daily_income(_user_product_id uuid, _perfect boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF _perfect THEN
    ...
    INSERT INTO public.transactions(user_id, amount, type, description)
    VALUES (v_user, v_bonus, 'daily_income',
            'Perfect game bonus (no bombs) from ' || v_product.name);
  END IF;
