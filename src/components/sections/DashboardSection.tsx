...
import { Wallet, TrendingUp, AlertCircle, CheckCircle, MessageCircle, XCircle } from "lucide-react";
...
const DashboardSection = ({ onScrollToSection }: DashboardSectionProps) => {
  const { user } = useAuth();
  ...
  return (
    <section id="dashboard" className="py-8 px-4 max-w-lg mx-auto space-y-6">
      ...
      <Card className="shadow-elevated bg-gradient-to-br from-slate-900 via-blue-950 to-slate-800 animate-fade-in border-2 border-blue-500/30">
        <CardContent className="p-6 text-center space-y-2">
          ...
          <Button 
            variant="secondary" 
            size="sm"
            onClick={() => onScrollToSection('account')}
      ...
      {(availableBalance || 0) < 300 ? (
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-bold text-white">Withdrawal Progress</p>
                </div>
                <p className="text-sm text-blue-200">ETB {(availableBalance || 0).toFixed(2)} / ETB 300.00</p>
              </div>
              <Button size="sm" onClick={() => onScrollToSection('products')} className="bg-blue-600 hover:bg-blue-700 text-white border-0">
                Buy Products
              </Button>
            </div>
            <div className="space-y-2">
              <div className="relative h-3 w-full overflow-hidden rounded-full bg-slate-700 border border-blue-500/30">
                <div className="h-full bg-gradient-to-r from-blue-600 to-cyan-500 transition-all duration-500" style={{ width: `${((availableBalance || 0) / 300) * 100}%` }} />
              </div>
              <p className="text-xs font-medium text-blue-200 text-center">Need ETB {(300 - (availableBalance || 0)).toFixed(2)} more to reach minimum withdrawal</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Alert className="animate-fade-in border-2 border-amber-500/50 bg-gradient-to-r from-slate-900 to-slate-800 shadow-elevated">
          ...
          <AlertDescription className="flex items-center justify-between">
            <div>
              <p className="font-bold text-white">Purchase a product to withdraw</p>
              <p className="text-sm text-slate-200 mt-1">You have ETB {(availableBalance || 0).toFixed(2)} available</p>
            </div>
            <Button size="sm" onClick={() => onScrollToSection('products')} className="ml-2 bg-blue-600 hover:bg-blue-700 text-white border-0">
              View Products
            </Button>
          </AlertDescription>
        </Alert>
        ...
        <Alert className="animate-fade-in border-2 border-green-500/50 bg-gradient-to-r from-slate-900 to-slate-800 shadow-elevated">
          ...
          <AlertDescription className="flex items-center justify-between">
            <div>
              <p className="font-bold text-white">You can now withdraw!</p>
              <p className="text-sm text-slate-200 mt-1">Available: ETB {(availableBalance || 0).toFixed(2)}</p>
            </div>
            <Button size="sm" onClick={() => onScrollToSection('account')} className="ml-2 bg-green-600 hover:bg-green-700 text-white border-0">
              Withdraw Now
            </Button>
          </AlertDescription>
        </Alert>
      ...
      {recentRecharges?.map((recharge) => (
        recharge.status === 'rejected' ? (
          <Alert key={recharge.id} className="animate-fade-in bg-gradient-to-r from-slate-900 to-slate-800 border-2 border-red-500 shadow-elevated">
            ...
            <AlertDescription>
              <p className="font-bold text-red-400 text-base">Recharge Request Rejected</p>
              <p className="text-sm text-slate-200 mt-2">Please pay the amount before requesting.</p>
              <p className="text-xs text-slate-300 mt-1 font-medium">Product: {recharge.products?.name} • ETB {recharge.amount}</p>
            </AlertDescription>
          </Alert>
          ...
          <Alert key={recharge.id} className="animate-fade-in bg-gradient-to-r from-blue-950 to-blue-900 border-2 border-blue-400 shadow-elevated">
            ...
            <AlertDescription>
              <p className="font-bold text-blue-300 text-base">Congratulations!</p>
              <p className="text-sm text-slate-100 mt-2">Your product is now working and generating income!</p>
              <p className="text-xs text-blue-200 mt-1 font-medium">{recharge.products?.name} • ETB {recharge.amount}</p>
            </AlertDescription>
          </Alert>
        ) : null
      ))}
      ...
      {recentWithdrawals?.map((withdrawal) => (
        withdrawal.status === 'rejected' ? (
          <Alert key={withdrawal.id} className="animate-fade-in bg-gradient-to-r from-slate-900 to-slate-800 border-2 border-red-500 shadow-elevated">
            <XCircle className="h-5 w-5 text-red-500" />
            <AlertDescription>
              <p className="font-bold text-red-400 text-base">Withdrawal Request Rejected</p>
              <p className="text-sm text-slate-200 mt-2">Please ensure you meet the withdrawal requirements.</p>
              <p className="text-xs text-slate-300 mt-1 font-medium">ETB {withdrawal.amount}</p>
              <p className="text-xs text-slate-400 mt-1">Requested: {format(new Date(withdrawal.created_at), 'MMM dd, yyyy HH:mm')}</p>
            </AlertDescription>
          </Alert>
          ...
          <Alert key={withdrawal.id} className="animate-fade-in bg-gradient-to-r from-blue-950 to-blue-900 border-2 border-blue-400 shadow-elevated">
            ...
            <AlertDescription>
              <p className="font-bold text-blue-300 text-base">Congratulations!</p>
              <p className="text-sm text-slate-100 mt-2">Your withdrawal has been approved!</p>
              <p className="text-xs text-blue-200 mt-1 font-medium">ETB {withdrawal.amount} • {withdrawal.bank_accounts?.bank_name}</p>
            </AlertDescription>
          </Alert>
        ) : null
      ))}
        ...
        <Card className="shadow-elevated animate-fade-in bg-gradient-to-br from-blue-950 via-slate-900 to-blue-900 border-2 border-blue-500/30">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <Button variant="secondary" size="sm" onClick={() => window.open(telegramContact.link, '_blank')} className="bg-blue-600 hover:bg-blue-700 text-white border-0">
                Join Now
              </Button>
            </div>
          </CardContent>
